...
mnist_triplet_train_loader = torch.utils.data.DataLoader(mnist_triplet_train_dataset, batch_size=batch_size, shuffle=True, **kwargs)
mnist_triplet_test_loader = torch.utils.data.DataLoader(mnist_triplet_test_dataset, batch_size=batch_size, shuffle=False, **kwargs)
mnist_mini_triplet_train_loader = torch.utils.data.DataLoader(mnist_mini_triplet_train_dataset, batch_size=batch_size, shuffle=True, **kwargs)
mnist_mini_triplet_test_loader = torch.utils.data.DataLoader(mnist_mini_triplet_test_dataset, batch_size=batch_size, shuffle=False, **kwargs)
...
n_epochs_B = 50
n_epochs_S = n_epochs_B*len(train_loader_B) // len(train_loader_S) + 1 if n_epochs_B*len(train_loader_B) % len(train_loader_S) else n_epochs_B*len(train_loader_B) // len(train_loader_S)
log_interval = 100
...
for epoch_B in range(0, n_epochs_B):
    for batch_idx_B, (data_B, _) in enumerate(train_loader_B):
        ...
        if batch_idx_B == len(train_loader_B)-1:
            ...
            print('[Big   Dataset] Epoch: {}/{}. Train      set: Average loss: {:.4f}'.format(epoch_B + 1, n_epochs_B, total_loss_B))
            if if_log:
        ...
        if batch_idx_S == len(train_loader_S) - 1:
            ...
            print('[Small Dataset] Epoch: {}/{}. Train      set: Average loss: {:.4f}'.format(epoch_S, n_epochs_S, total_loss_S))
            if if_log:
        ...
        if batch_idx_S == len(train_loader_S) - 1:
            ...
            for epoch_G in range(0,n_epochs_G):
                ...
                for i,data_fake in enumerate(grads_B_loader):
                    ...
                    print("  Epoch:{}/{} Batch:{} Fake: {:.4f}  Real: {:.4f}".format(epoch_G+1,n_epochs_G,i,out_fake.data.mean().item(),out_real.data.mean().item()))
                    optim_D.zero_grad()
                    d_loss.backward()
                    optim_D.step()
        ...
        if batch_idx_B == len(train_loader_B)-1:
            with torch.no_grad():
                ...
                print('[Big   Dataset] Epoch: {}/{}. Validation set: Average loss: {:.4f}'.format(epoch_B + 1, n_epochs_B, val_loss))
                if if_log:
        ...
        if batch_idx_S == len(train_loader_S) - 1:
            with torch.no_grad():
                ...
                for batch_idx, (data, _) in enumerate(test_loader_S):
                    ...
                    val_loss += loss_outputs.item()
                val_loss /= len(test_loader_B)
                print('[Small Dataset] Epoch: {}/{}. Validation set: Average loss: {:.4f}'.format(epoch_S + 1, n_epochs_S,
                                                                                    val_loss))
                if if_log:
